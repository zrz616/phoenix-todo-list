defmodule AppWeb.CoreComponents do
  ...
  attr :inputClass, :string, default: nil
  ...
  def label(assigns) do
    ~H"""
    <label for={@for} class={@labelClass || "block text-sm font-semibold leading-6 text-zinc-800"}>
      <%= render_slot(@inner_block) %>
    </label>
    """
  end
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between gap-6", @class]}>
      <div>
        <h1 class="text-8xl text-center font-thin text-rose-200">
          <%= render_slot(@inner_block) %>
        </h1>
